...
- **Password protection**: the first time you try to add, edit, or remove anything (stock, components, machines, categories), a password prompt appears. The password is set in `src/components/PasswordGateModal.jsx` (currently `NS3DR123`). Once entered correctly it's remembered in the browser's `localStorage` (key `ns3dr_authorized`), so it won't ask again on that device/browser until the site data or browser cache is cleared. Viewing data (browsing, searching, opening details) never requires the password — only changes do. This is a simple front-end gate, not real authentication — anyone with the password (or who inspects the page) can bypass it, so don't rely on it to protect sensitive data from a determined user. For real access control, add Supabase Auth later.
